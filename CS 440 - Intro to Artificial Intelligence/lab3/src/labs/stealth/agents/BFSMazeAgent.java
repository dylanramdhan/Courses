    ...
    extends MazeAgent
{
                       ...
                       StateView state)
    {
        ...
        Queue<Path> queue = new LinkedList<>(); // Queue to manage the nodes to visit
        ...
        visited.add(src);
            ...
            while (!queue.isEmpty()) 
            {
                Path node = queue.poll(); // Queue = start
                   ...
                   for (Vertex nearby : NearTheGold) // graph.getNeighbors(node)) 
                    { 
                        if (!visited.contains(nearby) && Valid(nearby, state)) 
                        {
                            ...
                            queue.add(new Path(nearby, 1f, node)); // Add the neighbor to the queue for future processing
                        }
                    }
